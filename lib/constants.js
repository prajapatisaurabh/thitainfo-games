...
// Timer configuration for race mode
export const TIMER_OPTIONS = [
  {
    value: "text-based",
    label: "Auto (based on text)",
    description: "Calculated from text length",
  },
  { value: 30, label: "30 seconds", description: "Quick race" },
  { value: 60, label: "1 minute", description: "Short race" },
  { value: 90, label: "1.5 minutes", description: "Medium race" },
  { value: 120, label: "2 minutes", description: "Standard race" },
  { value: 180, label: "3 minutes", description: "Long race" },
];

export const GRACE_PERIOD_SECONDS = 15; // Grace period after first finisher
export const AVERAGE_WPM_BASELINE = 40; // Used for text-based timer calculation
export const TIMER_BUFFER_MULTIPLIER = 1.5; // Safety buffer for text-based timer

// Calculate timer duration from text length
export const calculateTimerFromText = (text) => {
  const words = text.length / 5; // Industry standard: 5 chars = 1 word
  const minutes = words / AVERAGE_WPM_BASELINE;
  const seconds = minutes * 60 * TIMER_BUFFER_MULTIPLIER;
  return Math.ceil(Math.max(seconds, 30)); // Minimum 30 seconds
};
