  ...
  io.on("connection", (socket) => {
        ...
        await roomsCollection.updateOne(
          { roomId, "players.socketId": socket.id },
          {
            $set: {
              ...
              ...(finished && !room.players.find(p => p.socketId === socket.id)?.finished
    ...
    socket.on("player-finished", async (data) => {
      try {
        ...
        // (This may already be set via player-progress, but we update with final stats)
        await roomsCollection.updateOne(
          { roomId, "players.socketId": socket.id },
          {
            $set: {
        ...
        // First player to finish - they are the winner!
